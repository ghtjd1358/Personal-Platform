...
export const Lnb = ({ lnbItems, title, appName, logo, onLogout }) => {
    ...
    return (_jsxs("aside", { className: `app-lnb ${collapsed ? 'collapsed' : ''}`, "aria-label": "\uC8FC \uBA54\uB274", children: [_jsxs("div", { className: "app-lnb-header", children: [(logo || appName) && (_jsx("div", { className: "app-lnb-logo", onClick: () => handleNavigate('/'), children: isValidElement(logo)
                    ...
                    return (_jsx("div", { className: "app-lnb-item", children: item.children ? (_jsxs(_Fragment, { children: [_jsxs("button", { className: `app-lnb-item-btn ${expanded ? 'expanded' : ''}`, type: "button", onClick: () => toggleExpand(item.id), "aria-expanded": expanded, "aria-label": collapsed ? item.title : undefined, "aria-current": childActive ? 'page' : undefined, children: [item.icon && _jsx("span", { className: "app-lnb-icon", children: item.icon }), !collapsed && _jsx("span", { className: "app-lnb-text", children: item.title }), !collapsed && item.badge && (_jsx(Badge, { variant: BADGE_VARIANT_MAP[item.badge.variant ?? 'primary'] ?? 'primary', children: item.badge.text })), !collapsed && (_jsx("span", { className: "app-lnb-arrow", "aria-hidden": "true", children: expanded ? '▼' : '▶' }))] }), expanded && !collapsed && (_jsx("div", { className: "app-lnb-subitems", children: item.children.map((child) => {
                                    ...
                                    }) }))] })) : (_jsxs("button", { className: `app-lnb-item-btn ${itemActive ? 'active' : ''}`, type: "button", onClick: () => item.path && handleNavigate(item.path), "aria-label": collapsed ? item.title : undefined, "aria-current": itemActive ? 'page' : undefined, children: [item.icon && _jsx("span", { className: "app-lnb-icon", children: item.icon }), !collapsed && _jsx("span", { className: "app-lnb-text", children: item.title }), !collapsed && item.badge && (_jsx(Badge, { variant: BADGE_VARIANT_MAP[item.badge.variant ?? 'primary'] ?? 'primary', children: item.badge.text }))] })) }, item.id));
