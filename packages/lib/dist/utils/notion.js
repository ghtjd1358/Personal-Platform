...
export const NOTION_PROXY_URL = 'https://ujhlgylnauzluttvmcrz.supabase.co/functions/v1/notion-page';
/**
 ...
 */
export function proxyNotionImageUrl(url, block) {
    if (!url)
        return undefined;
    ...
    try {
        ...
        if (u.searchParams.has('X-Amz-Signature'))
            return url;
        const proxiable = u.hostname.endsWith('.amazonaws.com') ||
    ...
    try {
        ...
        if (block?.id)
            v2.searchParams.set('id', block.id);
 ...
 */
export function createNotionImageMapper(recordMap) {
    return (url, block) => {
        if (!url)
            return undefined;
 ...
 */
export async function fetchNotionRecordMap(notionUrl) {
    const pageId = extractNotionPageId(notionUrl);
    if (!pageId) {
        throw new Error('Notion 페이지 ID 를 URL 에서 추출하지 못했습니다.');
    }
    const res = await fetch(`${NOTION_PROXY_URL}?pageId=${encodeURIComponent(pageId)}`);
    const data = await res.json();
    if (!res.ok)
        throw new Error(data?.error || `Proxy ${res.status}`);
    if (!data?.recordMap)
        throw new Error('recordMap 누락');
