...
import { selectUser, Logo, LoadingSpinner, ErrorState, EmptyState, Badge } from '@sonhoseong/mfa-lib';
...
const ResumeDetailPage: React.FC = () => {
  ...
  const { resume, isLoading, error, isOwner, expandedItem, toggleExpanded } = useResumeDetail(id, currentUser?.id);

  if (isLoading) return <LoadingSpinner fullPage message="이력서를 불러오는 중" />;
  if (error || !resume) return (
    <ErrorState
      message={error || '이력서를 찾을 수 없습니다.'}
      backHref={`${LINK_PREFIX}/resumes`}
      backLabel="목록으로 돌아가기"
    />
  );
    ...
    <div className="public-resume-detail">
      <div className="public-resume-nav">
        <div className="public-resume-nav-inner">
          <Link to={`${LINK_PREFIX}/resumes`} className="back-link">← 이력서 목록</Link>
          {isOwner && (
            <Link to={`${LINK_PREFIX}/mypage/${id}`} className="btn btn-primary btn-sm">
              ✏️ 내 이력서 관리
            </Link>
          )}
        </div>
      ...
      <section className="hero">
        <div className="container">
          <div className="hero-content">
            <div className="hero-mark"><Logo customSize={120} /></div>
            <h1 className="hero-title">
              안녕하세요,<br />
              {resume.title || '개발자'}<br />
              <span className="highlight">{resume.name || resume.user?.name || '익명'}</span>입니다.
            </h1>
            {resume.summary && <p className="hero-desc">{resume.summary}</p>}
            <div className="contact-icons animate-visible">
      ...
      <section className="section experience">
        <div className="container">
                ...
                {resume.experiences.map((exp) => (
                  <div key={exp.id} className="timeline-item animate-visible">
                    <div className={`timeline-date ${!exp.is_current ? 'past' : ''}`}>
                      {formatYearMonth(exp.start_date)} - {formatYearMonth(exp.end_date, exp.is_current, true)}
                      <Badge
                        ...
                        <>
                          ...
                          {expandedItem === exp.id && (
                            <ul className="timeline-tasks">
                              {exp.tasks.map((task) => <li key={task.id}>{task.task}</li>)}
                            </ul>
                          )}
                        </>
                      )}
          ...
          {resume.projects && resume.projects.length > 0 && (
            <>
              <div className="section-header section-header--mt animate-visible">
                ...
                {resume.projects.map((proj) => (
                  <div key={proj.id} className="timeline-item animate-visible">
                    <div className={`timeline-date ${!proj.is_current ? 'past' : ''}`}>
                      {formatYearMonth(proj.start_date)} - {formatYearMonth(proj.end_date, proj.is_current, true)}
                    </div>
                      ...
                      {proj.tasks && proj.tasks.length > 0 && (
                        <>
                          ...
                          {expandedItem === proj.id && (
                            <ul className="timeline-tasks">
                              {proj.tasks.map((task) => <li key={task.id}>{task.task}</li>)}
                            </ul>
                          )}
                        </>
                      )}
          ...
          {(!resume.experiences?.length && !resume.projects?.length) && (
            <EmptyState description="등록된 경력/프로젝트가 없습니다." />
          )}
        </div>
      </section>
    </div>
