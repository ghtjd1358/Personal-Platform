...
const ResumeCard: React.FC<ResumeCardProps> = ({ resume }) => {
  ...
  return (
    <Link to={`${LINK_PREFIX}/user/${resume.user_id}`} className="resume-card">
      <div className="resume-card__image">
        {resume.profile_image ? (
          <img
            src={resume.profile_image}
            alt={`${resume.user?.name || '사용자'}의 프로필`}
            loading="lazy"
          />
        ) : (
          <div className="resume-card__image-placeholder">
            <img
              src={resume.user?.avatar_url || defaultAvatar}
              alt={resume.user?.name || '사용자'}
              className="resume-card__avatar"
            />
          </div>
        )}
      </div>
      <div className="resume-card__content">
        <div className="resume-card__header">
          <img
            src={resume.user?.avatar_url || defaultAvatar}
            alt={resume.user?.name || '사용자'}
            className="resume-card__user-avatar"
          />
          <span className="resume-card__user-name">{resume.user?.name || '익명'}</span>
        </div>
