 ...
 */
export async function uploadImage(file: File, folder: string = 'portfolio'): Promise<ApiResponse<UploadResult>> {
  try {
 ...
 * 여러 이미지를 동시에 업로드합니다.
 */
export async function uploadImages(files: File[], folder: string = 'portfolio'): Promise<ApiResponse<UploadResult[]>> {
  try {
