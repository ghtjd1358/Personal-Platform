...
const PortfolioModal: React.FC<PortfolioModalProps> = ({ portfolioId, onClose }) => {
  const { portfolio, isLoading } = usePortfolioModal(portfolioId);
  ...
  return (
    <div className="pm-backdrop" onClick={handleBackdropClick}>
      <div className="pm-modal" role="dialog" aria-modal="true">
        <Button.Icon aria-label="닫기" className="pm-close" onClick={onClose}>
        ...
        {isLoading ? (
          ...
          <div className="pm-error">
            <ErrorState message="포트폴리오를 찾을 수 없습니다." />
          </div>
        ) : (
              ...
              <aside className="pm-left">
                ...
                {(() => {
                  ...
                  if (!hasMeta) {
                    return (
                      <EmptyState
                        icon="✶"
                        description="아직 상세 정보가 등록되지 않은 프로젝트입니다."
                      />
                    );
                  }
                      ...
                      {hasTech && (
                        <div className="pm-meta-tech">
                          ...
                          <div className="pm-chips">
                            ...
                            {(portfolio.techStack as any[]).map((tech) => (
                              <Badge key={tech.id} variant="default" className="pm-chip">{tech.name}</Badge>
                            ))}
                          </div>
                        </div>
                      )}
                ...
                {(portfolio.demo_url || portfolio.github_url) && (
                  <div className="pm-links">
                    {portfolio.demo_url && (
                      <a href={portfolio.demo_url} target="_blank" rel="noopener noreferrer" className="pm-link pm-link--primary">
                    ...
                    {portfolio.github_url && (
                      <a href={portfolio.github_url} target="_blank" rel="noopener noreferrer" className="pm-link pm-link--secondary">
              ...
              <div className="pm-right">
                <header className="pm-title-block">
                  <div className="pm-eyebrow">
                    {portfolio.category?.name && <Badge variant="info" className="pm-eyebrow-cat">{portfolio.category.name}</Badge>}
                    {portfolio.is_featured && <Badge variant="primary" className="pm-eyebrow-featured">★ FEATURED</Badge>}
                  </div>
                  <h1 className="pm-title">{portfolio.title}</h1>
                  {portfolio.short_description && <p className="pm-lede">{portfolio.short_description}</p>}
                </header>

                {portfolio.notion_url ? (
                  <a href={portfolio.notion_url} target="_blank" rel="noopener noreferrer" className="pm-notion-cta">
                    <div className="pm-notion-cta-meta">
                      ...
                      <p className="pm-notion-cta-sub">과정 · 의사결정 · 트러블슈팅 까지 — 노션 페이지로 새 탭에서 엽니다</p>
                    </div>
                  ...
                  <>
                    {portfolio.detail?.overview && <section className="pm-section"><h2 className="pm-h2">개요</h2><p className="pm-paragraph">{portfolio.detail.overview}</p></section>}
                    {portfolio.detail?.challenge && <section className="pm-section"><h2 className="pm-h2">도전 과제</h2><p className="pm-paragraph">{portfolio.detail.challenge}</p></section>}
                    {portfolio.detail?.solution && <section className="pm-section"><h2 className="pm-h2">접근 방법</h2><p className="pm-paragraph">{portfolio.detail.solution}</p></section>}
                    {portfolio.detail?.outcome && <section className="pm-section"><h2 className="pm-h2">성과</h2><p className="pm-paragraph">{portfolio.detail.outcome}</p></section>}
                ...
                {portfolio.tags && portfolio.tags.length > 0 && (
                  <div className="pm-tags">
                    {portfolio.tags.map((t) => <Badge key={t.id} variant="default" className="pm-tag">#{t.tag}</Badge>)}
                  </div>
                )}
              </div>
            </div>
